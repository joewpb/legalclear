...
| **Decision 6 attorney confirmation** | The § 48.183 posted-service later-of rule is live (three worked-example tests). SUPERSEDED by Decision 11 (2026-08-18) — the attorney-review requirement was deliberately traded for the educational-framing output standard. The 2026-08-18 legal answer stands as research input; attribution was never supplied. |
...
- **Statutes corpus was stubs until 2026-08-18 (corrected).** The ch. 34/83 `statutes` rows ingested by the original `scripts/ingest_statutes.py` held heading + first-subsection-label + History footer — the statutory body was dropped by a span-terminated parser bug (first `</span>` inside SectionBody ended capture). Discovered 2026-08-18 when the § 83.60 row measured 147 chars vs ~2,900 official. The ch. 34 citation pilot's verification ("citation → row → text present → source_url") therefore proved PRESENCE, not COMPLETENESS — the claim "verified against owned text" was misleading in effect and is hereby corrected. Corpus rebuilt 2026-08-18 for ch. 34 (17/17) + ch. 83 (74/74) with a mode-based parser; validation replaced with: min-length report (sections under 120 chars named; genuinely-short statutes verified against the chapter page and ingested), empty-body skip (old row untouched on failed fetch), and a 10% sample ratio check against official per-section pages (9/9 ratios 0.99–1.08). Scripts: `scripts/ingest_statutes.py` (fixed), `scripts/dryrun_statutes.py`, `scripts/sample_ratio_check.py`.
- **Rate-limit tiers (2026-08-17, RL-2):** LLM-calling routes = `10/minute` (wills_trusts, property_casualty, small_claims ×2, discovery, criminal, police_report ×2, chat, intake, forms/suggest, attorney_referral/intake). Deterministic routes = `60/minute` (case_law/search, packet/build — Postgres round-trips, no LLM; VERIFIED no model calls). Expungement router routes carry NO limit: `/eligibility` is deterministic disqualifier matching and `/generate` is Stripe+PDF — neither calls an LLM (VERIFIED 2026-08-17); the LLM `ExpungementAgent` runs only in the `/api/upload` pipeline in routes.py. **CORRECTION 2026-08-20 — authoritative enumeration: 11 of 12 LLM surfaces are rate-limited. The exception is the upload pipeline itself (`routes.py:249` `/api/upload` + `process_document:299`): classifier, risk_scanner, scanner, explainer, form_guide, and the expungement analyzer all run there with NO limiter — the heaviest LLM surface in the product is unlimited. The 2026-08-18 AUDIT sweep's "every LLM surface rate-limited" claim was wrong; S1-8's "3 of ~13" predated RL-2. Both corrected. opinion_retrieval.py's LLM query synthesis has no live call site (latent). Fix is one decorator on the upload route — recorded, not applied; Joe rules whether it rides G2 or lands standalone.**
...
- **`family_law` rule set is KNOWN-BROKEN, not loaded (2026-08-20).** The set is a single 155,089-char row: the official PDF parse found no RULE headings, so the text is unsplit and no individual family-law rule citation can resolve. Recorded as a known-broken rule set — deliberately NOT fixed, because there is no family law module and no consumer. It must not be treated as a loaded/usable rule set in any future citation work; if a family law module ever lands, the parse fix (heading detection) is a prerequisite.
