...
| 8 | UPL Wall & Escalation Enforcement | COMPLETE | 2026-05-19 | upl.py (5 triggers, FATAL_CONFIDENCE_THRESHOLD=0.90, URGENT_HOURS=72); EN/ES disclaimers 3 levels; /api/analysis/* router; 25 tests pass. |
...
- **Next engineering task:** wire module #2 of the opinion-retrieval pattern. Per-module recipe: (1) recon that module's extracted-result shape, (2) write `derive_*_tags` against the verified vocabulary, (3) call `get_relevant_opinions()` at the response-construction point, (4) reuse `OpinionCard.tsx` as-is. Corpus alignment favors a criminal-procedure-adjacent module over Small Claims (thin coverage, `small_claims`=1).
...
10. **Terms of Service** — draft + review (parallel workstream, gates launch).
11. **Tech E&O insurance** — obtain before public launch.
