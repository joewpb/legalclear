...
| Finding 3 — auth (testkey default, unauthed endpoints, IDOR) | **MOSTLY CLOSED** | `testkey123` default GONE from config.py (startup fail-fast); IDOR fixed (A-phase, idor tests green); rate limiting: 11 of 12 LLM surfaces limited at 10/min (RL-2, tiers by cost) — **CORRECTION 2026-08-20: the upload pipeline (`routes.py:249 /api/upload` + `process_document:299` — classifier + risk_scanner + scanner + explainer + form_guide + expungement analyzer, the heaviest LLM surface in the product) has NO limiter. The sweep's "every LLM surface rate-limited" claim was wrong.** REMAINS: S1-8 (key ships in public bundle — architectural, Decision 10 defers session tokens to G2) |
...
5. **The UPL wall is not uniformly enforced.** ~~Criminal Procedure and Discovery Motion streaming **success** paths emit no server-injected disclaimer — the canonical disclaimer appears only on error paths; success relies on the LLM writing a `disclaimer` field inside its own JSON (`agents/criminal_procedure.py:150-206`, `agents/discovery_motion.py:175-210`).~~ **UPDATED 2026-08-18 (re-verification): U1 is CLOSED.** Both streams now yield a server-injected `event: disclaimer` with the canonical text on the success path (`agents/criminal_procedure.py:176-179`, `agents/discovery_motion.py:194-197`, via `get_disclaimer` → `core/upl.py`), with tests (`test_criminal_disclaimer_sse.py`, `test_discovery_disclaimer_sse.py`). The attorney-referral router is also inside the wall (`routers/attorney_referral.py:24,130,143,159`). The earlier U1 finding was stale — the fix landed without the audit being updated.
...
| 7 | UPL wall + escalation on every output | VL P8, CLAUDE.md | `core/upl.py`, `core/escalation.py` | n/a | applied per-router | n/a | n/a | `test_upl.py`, `test_pc_upl.py` | **CLOSED 2026-08-18** — all four U1-named surfaces (criminal, discovery, wills/trusts, small claims) + attorney_referral verified inside the wall (see finding 5 update) |
...
| 9 | Criminal Procedure explainer | SL §2 | `agents/criminal_procedure.py` | n/a | `/api/criminal/explain` (rate-limited, no auth) | `CriminalProcedureExplainer.tsx` explicit submit `:439` | ✓ tile | none unit-level | **WORKING** (U1 closed 2026-08-18) |
| 10 | Discovery Motion analyzer | SL §2 | `agents/discovery_motion.py` | n/a | `/api/discovery/analyze` | `DiscoveryMotionAnalyzer.tsx` | ✓ tile | none unit-level | **WORKING** (U1 closed 2026-08-18) + silent risk-parse swallow (`discovery_motion.py:209-210`) |
...
| S1-8 | `settings.API_KEY` shipped in the frontend bundle as `VITE_API_KEY` | frontend `.env.production` → public JS bundle; sent as `x-api-key` by `api.js` + pages | Every API-key-gated route — including every LLM-calling one — is invocable by anyone who reads the bundle. Rotating the key does not fix it: a public SPA cannot hold a secret. **Gating raises the bar against scanners; it is NOT access control.** | Scoped 2026-08-17 for Joe's pick: Anthropic spend caps/alerts; per-IP rate limits; short-lived server-issued session tokens | All key-gated routes incl. all LLM endpoints | Needs Joe's call |
