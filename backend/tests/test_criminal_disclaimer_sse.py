"""B4b-4 — criminal_procedure.py disclaimer: typed SSE event on success + error paths.
...
mid-stream error path, while leaving the embedded ``disclaimer`` field inside
the JSON payload/error frame unchanged (backward compat with older clients).
"""
import asyncio
import json
import os
