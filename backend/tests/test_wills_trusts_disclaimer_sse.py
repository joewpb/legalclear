"""B4b-5 / B4e-b — wills_trusts.py disclaimer: typed SSE event on
success + error paths.

Pins that WillsTrustsExplainer.explain emits a typed ``event: disclaimer``
SSE frame on the normal completion path and on any error path that follows
emitted content, while leaving the embedded ``disclaimer`` field inside the
JSON payload/error frame unchanged (backward compat with older clients).
Per Decision 5, an error that fires before any substantive content has been
streamed carries no disclaimer event.
"""
import asyncio
import json
import os
...
def test_explain_error_before_content_omits_disclaimer():
...
def test_explain_error_after_content_emits_typed_disclaimer():
