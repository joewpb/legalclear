...
class DeadlineRule(TypedDict):
    ...
    consequence: str
    note: str | None
    # I-3a — which claim regime(s) (see src.core.claim_regime) this clock
    # applies under. None (the default for non-P&C rules) means "not
    # regime-scoped". A P&C clock declares an explicit tuple even when it
    # applies to both regimes — see pc_rule_keys_for_regime below, which
    # treats "unknown" as selecting nothing, never a guess.
...
RULES: dict[str, DeadlineRule] = {
    ...
    "pc_acknowledge_claim": {
        ...
        "day_counting":             "calendar",
        "explicitly_business_days": False,
        "required_anchors":         ("claim_noticed",),   # 7 days after receiving the claim communication
        "governing_rule":           "Fla. Stat. § 627.70131(1)(a)",
        "severity":                 "medium",
        "consequence": (
            "The insurer must review and acknowledge receipt of a claim "
            "communication within 7 calendar days, unless payment is made "
            "within that period or the delay is caused by factors beyond "
            "the insurer's control. The acknowledgment must provide claim "
            "forms and instructions. Missing this deadline does not bar the "
            "claim, but is evidence of mishandling if a bad-faith or Civil "
            "Remedy Notice dispute develops later."
        ),
        "note": (
            "Does not apply to communications necessary only to provide "
            "forms and instructions to a claimant represented by counsel."
        ),
        "regimes":                  ("pre", "post"),
    },
    ...
    "pc_estimate_delivery": {
        ...
        "day_counting":             "calendar",
        "explicitly_business_days": False,
        "required_anchors":         ("estimate_generated",),   # 7 days after the adjuster generates the estimate
        "governing_rule":           "Fla. Stat. § 627.70131(3)(e)",
        "severity":                 "medium",
        "consequence": (
            "The insurer must send the policyholder a copy of any detailed "
            "estimate of the amount of the loss within 7 days after the "
            "estimate is generated by the insurer's adjuster. There is no "
            "requirement to create an estimate that is not reasonably "
            "necessary to the claim investigation."
        ),
        "note": (
            "This clock runs from the date the adjuster generates the "
            "estimate, not from the date of loss — it cannot be computed "
...
def pc_rule_keys_for_regime(regime: str | None) -> list[str]:
    """Return the P&C rule keys (``pc_*``) that apply under `regime`.

    "unknown" always returns an empty list — a clock declared for a subset
    of regimes must never be selected when the applicable regime cannot be
    determined (see src.core.claim_regime.resolve_regime); it must escalate
    instead of guessing. ``None`` (no regime resolved for this call at all,
    e.g. no session on file) returns every declared P&C rule, preserving
    the pre-I-3a behavior of computing every P&C clock unconditionally.
    Every current P&C clock is declared for both "pre" and "post" — there is
    no regime-varying clock yet — so filtering by "pre" or "post" today
    yields the same set; the mechanism exists so a future regime-specific
    clock can be added by data declaration alone.
    """
    pc_keys = [key for key in RULES if key.startswith("pc_")]
    if regime is None:
        return pc_keys
    if regime == "unknown":
        return []
    return [key for key in pc_keys if regime in (RULES[key].get("regimes") or ())]
